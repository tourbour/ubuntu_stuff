...
echo -en "${BGGREEN} Ставим утилиту для подстройки Gnome"
sleep 5
sudo apt install -y gnome-tweak-tool

# ВЕРНУТЬ ДОПОЛНЕНИЯ ДЛЯ GNOME:
echo -en "${BGGREEN} Включаем расширения для Gnome"
sleep 5
sudo apt install -y chrome-gnome-shell

# ТЕМЫ ИКОНОК NUMIX
echo -en "${BGGREEN} Добавляем репозиторий с темами и устанавливаем тему и значки Numix"
sleep 5
sudo add-apt-repository -y ppa:numix/ppa
sudo apt install -y numix-gtk-theme numix-icon-theme-circle

# ПОДДЕРЖКА 32-БИТ БИБЛИОТЕК
echo -en "${BGGREEN} Включаем поддержку 32-битных приложений и ставим необходимый минимум"
sleep 5
...
# КОДЕКИ, ДОПОЛНЕНИЯ
echo -en "${BGGREEN} Устанавливаем кодеки, шрифты и прочие надобности"
sleep 5
...
# АРХИВАТОРЫ
echo -en "${BGGREEN} Устанавливаем набор различных архиваторов"
sleep 5
sudo apt install -y p7zip-rar rar unrar unace arj cabextract

# ПАКЕТ JDK, JRE И ПЛАГИН ДЛЯ БРАУЗЕРА
echo -en "${BGGREEN} Устанавливаем Java для системы и браузера"
sleep 5
sudo add-apt-repository -y ppa:webupd8team/java
sudo apt install -y oracle-java8-installer

# ОБНОВЛЕНИЕ МИКРОКОДА INTEL
echo -en "${BGGREEN} Обновляем микрокод процессора"
sleep 5
...
echo -en "${BGGREEN} Добавляем репозиторий с проприетарными драйверами Nvidia"
sleep 5
sudo add-apt-repository -y ppa:graphics-drivers/ppa

#TMP ПАПКУ В ОПЕРАТИВНУЮ ПАМЯТЬ
echo -en "${BGGREEN} Помещаем папку для временных файлов в оперативную память"
sleep 5
